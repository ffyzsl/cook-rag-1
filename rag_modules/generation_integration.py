...
class GenerationIntegrationModule:
    ...
    def setup_llm(self):
        ...
        :return:
        """
        logger.info(f"正在初始化阿里云百炼 LLM: {self.model_name}")
    ...
    def _build_context(self, docs: List[Document], max_length: int = 2000) -> str:
        ...
        for i, doc in enumerate(docs,1):
            metadate_info = f"[食谱{i}]"
            if 'dish_name' in doc.metadata:
                metadate_info += f" {doc.metadata['dish_name']}"
            if 'category' in doc.metadata:
                metadate_info += f" | 分类: {doc.metadata['category']}"
            if 'difficulty' in doc.metadata:
                metadate_info += f" | 难度: {doc.metadata['difficulty']}"

            doc_text = f"{metadate_info}\n{doc.page_content}"
...
回答:""")
    ...
    def query_router(self, query: str) -> str:
        ...
        # 安全防线 如果大模型没有听话返回这三个词，强制回退为 general
    ...
    def query_rewrite(self, query: str) -> str:
        ...
        :return:
        """
        # 注意这里用的是 PromptTemplate 而不是 ChatPromptTemplate，
        # 因为我们只是想完成一个单轮的文本补全任务。
